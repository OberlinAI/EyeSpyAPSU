...
makeLimitPlots <- function(logs, measures) {
  ...
  print(ggplot(data=data, aes(x = Limit, y = Average, linetype=Measure)) + 
          geom_line() +
          scale_x_continuous(name="Pixels", breaks = seq(0, 1, 0.1)) +
...
LOGFILE_PREFIX <- paste0("results_.*_ZFNet_", UNDERSAMPLING_RATIO, "Ratio_.*min.*")
...
logs_zfnet_test <- parseLogs(counties, UNDERSAMPLING_RATIO)
logs_zfnet_test_averages <- averageLogs(logs_zfnet_test)

# print the results
logs_zfnet_test_averages[logs_zfnet_test_averages$Limit == THRESHOLD,]

# plot the performances as a function of the decision threshold
logsLong_zfnet_test <- parseLogsLong(counties, UNDERSAMPLING_RATIO)
makeLimitPlots(logsLong_zfnet_test, c("Recall_P", "Recall_N", "BalancedAccuracy", "MCC"))
