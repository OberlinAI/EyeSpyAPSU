...
parseLogs <- function(counties, ratio) {
  # read in all of the files
  frames <- list()
  count <- 1
  
  for (county in counties) {
    prefix <- paste0("results_county", county, "_.*_", ratio, "Ratio_.*min.*")
    ...
    for (file in files) {
      ...
      trainPerc <- gsub("min", "", nameSplit[6])
...
parseLogsLong <- function(counties, ratio) {
  for (county in counties) {
    prefix <- paste0("results_county", county, "_.*", ratio, "Ratio_.*min.*")
    files <- list.files(pattern = prefix)
...
# this function creates a line of each performance measure as a function of the probability threshold (output by the CNN)
# used to decide  whether an image contains a windmill
makeLimitPlots <- function(logs, measures) {
  ...
  print(ggplot(data=data, aes(x = Limit, y = Average, linetype=Measure)) + 
          geom_line() +
          scale_x_continuous(name="Threshold", breaks = seq(0, 1, 0.1)) +
          scale_y_continuous(name="Performance") +
          ggtitle(paste0("Average Performance per Prediction Threshold")) +
          theme_bw() +
...
UNDERSAMPLING_RATIO <- 9                              # TODO: change this to your undersampling ratio (number of negative instances per positive instance)
THRESHOLD <- round(1 / (1 + UNDERSAMPLING_RATIO), 3)  # the probability threshold to use to decide whether an image contains a windmill
LOGFILE_PREFIX <- paste0("results_.*_VGG_", UNDERSAMPLING_RATIO, "Ratio_.*min.*")
...
logs_vgg_test <- parseLogs(counties, UNDERSAMPLING_RATIO)
logs_vgg_test_averages <- averageLogs(logs_vgg_test)

# print the results
logs_vgg_test_averages[logs_vgg_test_averages$Limit == THRESHOLD,]

# plot the performances as a function of the decision threshold
logsLong_vgg_test <- parseLogsLong(counties, UNDERSAMPLING_RATIO)
makeLimitPlots(logsLong_vgg_test, c("Recall_P", "Recall_N", "BalancedAccuracy", "MCC"))
