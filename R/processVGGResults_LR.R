...
makeLRBarPlots <- function(logs1, logs5, logs9, measures, zoom) {
  ...
  print(ggplot(data=subdata, aes(x = LR, y = Average, fill=Ratio)) +
          ...
          ggtitle("VGG-16 Grid Search Hyperparameter Tuning") +
          xlab("Learning Rate") + 
...
# set some constants
DATAFOLDER <- "/mnt/windmills/results/VGG/"  # TODO: change this to your results directory
...
LOGFILE_PREFIX <- paste0("results_VGGTrainTest_", UNDERSAMPLING_RATIO, "Ratio")
logs_vgg_1ratio <- parseLogs(LOGFILE_PREFIX, UNDERSAMPLING_RATIO)
logsLong_vgg_1ratio <- parseLogsLong(LOGFILE_PREFIX, UNDERSAMPLING_RATIO)

# get the undersampling ratio 1:5 (windmill:nonwindmill) results
UNDERSAMPLING_RATIO = 5
LOGFILE_PREFIX <- paste0("results_VGGTrainTest_", UNDERSAMPLING_RATIO, "Ratio")
logs_vgg_5ratio <- parseLogs(LOGFILE_PREFIX, UNDERSAMPLING_RATIO)
logsLong_vgg_5ratio <- parseLogsLong(LOGFILE_PREFIX, UNDERSAMPLING_RATIO)

# get the undersampling ratio 1:9 (windmill:nonwindmill) results
UNDERSAMPLING_RATIO = 9
LOGFILE_PREFIX <- paste0("results_VGGTrainTest_", UNDERSAMPLING_RATIO, "Ratio")
logs_vgg_9ratio <- parseLogs(LOGFILE_PREFIX, UNDERSAMPLING_RATIO)
logsLong_vgg_9ratio <- parseLogsLong(LOGFILE_PREFIX, UNDERSAMPLING_RATIO)

# get the appropriate thresholds for each undersampling ratio
logs_vgg_1ratio_thresholded = logs_vgg_1ratio[logs_vgg_1ratio$Limit == 0.5,]
logs_vgg_5ratio_thresholded = logs_vgg_5ratio[logs_vgg_5ratio$Limit == 0.167,]
logs_vgg_9ratio_thresholded = logs_vgg_9ratio[logs_vgg_9ratio$Limit == 0.1,]

# print the best Balanced Accuracy performance for each undersampling ratio
logs_vgg_1ratio_thresholded[logs_vgg_1ratio_thresholded$BalancedAcc == max(logs_vgg_1ratio_thresholded$BalancedAcc),]
logs_vgg_5ratio_thresholded[logs_vgg_5ratio_thresholded$BalancedAcc == max(logs_vgg_5ratio_thresholded$BalancedAcc),]
logs_vgg_9ratio_thresholded[logs_vgg_9ratio_thresholded$BalancedAcc == max(logs_vgg_9ratio_thresholded$BalancedAcc),]

# create the box plots (from Appendix C)
makeLRBarPlots(logsLong_vgg_1ratio, logsLong_vgg_5ratio, logsLong_vgg_9ratio, c("BalancedAccuracy"), FALSE)
makeLRBarPlots(logsLong_vgg_1ratio, logsLong_vgg_5ratio, logsLong_vgg_9ratio, c("BalancedAccuracy"), TRUE)